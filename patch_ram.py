...
# Find RAM's bert.py - check local stag folder first, then site-packages
script_dir = os.path.dirname(os.path.abspath(__file__))
local_stag_bert = os.path.join(script_dir, 'stag', 'ram', 'models', 'bert.py')

site_packages = site.getsitepackages()[0] if hasattr(site, 'getsitepackages') else os.path.join(sys.prefix, 'lib', 'site-packages')
candidates = [
    local_stag_bert,
    os.path.join(site_packages, 'ram', 'models', 'bert.py'),
    os.path.join(sys.prefix, 'Lib', 'site-packages', 'ram', 'models', 'bert.py'),
    os.path.join(sys.prefix, 'lib', f'python{sys.version_info.major}.{sys.version_info.minor}', 'site-packages', 'ram', 'models', 'bert.py'),
]

bert_paths_to_patch = [p for p in candidates if os.path.exists(p)]
if not bert_paths_to_patch:
    print("RAM bert.py not found - skipping patch (AI tagger may not work)")
    sys.exit(0)

bert_path = bert_paths_to_patch[0]
